...
int		KdWindow::s_mouseWheelVal = 0;
...
bool KdWindow::OpenFileDialog(std::string& filepath, const std::string& title, const char* filters)
{
	// 現在のカレントディレクトリ保存
...
bool KdWindow::SaveFileDialog(std::string& filepath, const std::string& title, const char* filters, const std::string& defExt)
{
	// 現在のカレントディレクトリ保存
	auto current = std::filesystem::current_path();
	// ファイル名のみ
...
LRESULT KdWindow::WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEWHEEL:
		{
			s_mouseWheelVal = (short)HIWORD(wParam);
		}
		break;
	// Xボタンが押された
	case WM_CLOSE:
