...
void KdEffectShader::DrawLine(const KdVec3& p1, const KdVec3& p2, const Math::Color & color)
{
	// 定数バッファ書き込み
	m_cb0.Write();
...
void KdEffectShader::DrawMesh(const KdMesh* mesh, const std::vector< KdSptr<KdMaterial>>& materials)
{
	...
	for (UINT subi = 0; subi < mesh->GetSubsets().size(); subi++)
	{
		...
		else if (materials.size() >= 1) // 範囲外だがマテリアルデータがあるとき
		{
			// 最初のマテリアルを使用
