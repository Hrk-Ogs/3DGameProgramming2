...
static void MatrixMirrorZ(KdMatrix& mat)
{
	// 回転のZミラーリング
	mat._13 *= -1;
	mat._23 *= -1;
...
std::shared_ptr<KdGLTFModel> KdLoadGLTFModel(const std::string& path)
{
	...
	for (UINT nodei = 0; nodei < destModel->Nodes.size(); nodei++)
	{
		...
		if (model.nodes[nodei].rotation.size() != 0)
		{
			...
			mR.CreateFromQuaternion(q);
		}
		// 移動
		if (model.nodes[nodei].translation.size() != 0)
		{
			mT.CreateTranslation(
	...
	// ルートノードのみの参照リスト
	//----------------------------------
	for (int idx : model.scenes[0].nodes)
	{
	...
	{
		// 行列計算用 再帰関数
		std::function<void(KdGLTFNode*, const KdMatrix*)> rec = [&rec, destModel](KdGLTFNode* node, const KdMatrix* parentMat)
		{
			if (parentMat) {
			...
			{
				rec(&destModel->Nodes[childIdx], &node->WorldTransform);
			}
		};
	...
	{
		// 配列確保
//		destModel->BoneNodes.resize(model.skins[0].joints.size());
		...
		for (UINT ji = 0; ji < model.skins[0].joints.size(); ji++)
		{
			...
			// ノードの参照を記憶しておく
//			destModel->BoneNodes[ji] = &destModel->m_nodes[nodeIdx];
			// ボーンリスト内でのIndex
//			destModel->BoneNodes[ji]->BoneNodeIndex = ji;
			...
			// 変換行列へ変換
			boneNode->WorldTransform = invBindMat;
			boneNode->WorldTransform.Inverse();
		}
...
//			std::vector<KdMeshVertexEx_Skin>	VerticesEx_Skin;
			KdMeshVertexData					VertexData;
		...
		for (size_t pri = 0; pri < model.meshes[msi].primitives.size(); pri++)
		{
			...
			// 頂点バッファ
			{
				// 座標
				{
					...
					size_t count = posGetter.GetAccessor()->count;
					destPrimitive->VertexData.PosList.resize(count);
//					destPrimitive->VertexData.TangentList.resize(count);

					KdVec3 pos;
					for (UINT vi = 0; vi < posGetter.GetAccessor()->count; vi++) {
						auto& ver = destPrimitive->VertexData.PosList[vi];

						if (posGetter.GetAccessor()->type != TINYGLTF_TYPE_VEC3) {
							assert(0 && "この頂点形式には対応してません");
						}

						ver.x = posGetter.GetValue_Float(vi * 3 + 0);
						ver.y = posGetter.GetValue_Float(vi * 3 + 1);
						ver.z = posGetter.GetValue_Float(vi * 3 + 2) * -1;
					}
				}
				...
				{
					destPrimitive->VertexData.NormalList.resize(destPrimitive->VertexData.PosList.size());

					// 法線ゲッター
					GLTFBufferGetter normalGetter(&model, srcPrimitive.attributes["NORMAL"]);

					for (UINT vi = 0; vi < destPrimitive->VertexData.NormalList.size(); vi++) {
						auto& nor = destPrimitive->VertexData.NormalList[vi];
				...
				{
					destPrimitive->VertexData.UVList.resize(destPrimitive->VertexData.PosList.size());

					// UVゲッター
					GLTFBufferGetter uvGetter(&model, srcPrimitive.attributes["TEXCOORD_0"]);


					for (UINT vi = 0; vi < destPrimitive->VertexData.UVList.size(); vi++) {
						auto& uv = destPrimitive->VertexData.UVList[vi];
				...
				{
					destPrimitive->VertexData.ColorList.resize(destPrimitive->VertexData.PosList.size());

					// 色ゲッター
					GLTFBufferGetter colorGetter(&model, srcPrimitive.attributes["COLOR_0"]);

					for (UINT vi = 0; vi < destPrimitive->VertexData.ColorList.size(); vi++)
					{
						...
						destPrimitive->VertexData.ColorList[vi] = color.RGBA().v;
					}
				}
				...
				if (srcPrimitive.attributes.count("JOINTS_0") > 0)
				{
					...
					destPrimitive->VertexData.SkinIndexList.resize(jointGetter.GetAccessor()->count);

					for (UINT vi = 0; vi < destPrimitive->VertexData.SkinIndexList.size(); vi++)
					{
						// ※IndexはボーンリストのIndexになる(ノード全体ではない)
						auto& skinIndex = destPrimitive->VertexData.SkinIndexList[vi];
				...
				if (srcPrimitive.attributes.count("WEIGHTS_0") > 0)
				{
					...
					destPrimitive->VertexData.SkinWeightList.resize(weightGetter.GetAccessor()->count);

					for (UINT vi = 0; vi < destPrimitive->VertexData.SkinWeightList.size(); vi++) {
						auto& skinWei = destPrimitive->VertexData.SkinWeightList[vi];
			...
			// 足りない場合は、データを作成
			//---------------------------
			if (destPrimitive->VertexData.NormalList.size() == 0)
			{
				destPrimitive->VertexData.NormalList.resize(destPrimitive->VertexData.PosList.size());

				DirectX::ComputeNormals(&destPrimitive->Faces[0].Idx[0], destPrimitive->Faces.size(),
										&destPrimitive->VertexData.PosList[0], destPrimitive->VertexData.PosList.size(),
										DirectX::CNORM_DEFAULT, &destPrimitive->VertexData.NormalList[0]);
			}

			if (destPrimitive->VertexData.TangentList.size() == 0)
			{
				destPrimitive->VertexData.TangentList.resize(destPrimitive->VertexData.PosList.size());

				DirectX::ComputeTangentFrame(&destPrimitive->Faces[0].Idx[0], destPrimitive->Faces.size(),
					&destPrimitive->VertexData.PosList[0],
					&destPrimitive->VertexData.NormalList[0],
					&destPrimitive->VertexData.UVList[0],
					destPrimitive->VertexData.PosList.size(),
					&destPrimitive->VertexData.TangentList[0], nullptr);
			}

			if (destPrimitive->VertexData.ColorList.size() == 0)
			{
				destPrimitive->VertexData.ColorList.resize(destPrimitive->VertexData.PosList.size());

				for (auto&& n : destPrimitive->VertexData.ColorList)
				{
					n = 0xFFFFFFFF;
				}
			}
		...
		for (UINT pi = 0; pi < tempPrimitives.size(); pi++)
		{
			...
			// 頂点バッファ合成
			destNode->Mesh.VertexData.Combine(prim->VertexData);
			/*
			if (prim->Vertices.size() >= 1) {
				destNode->Mesh.Vertices.insert(destNode->Mesh.Vertices.end(), prim->Vertices.begin(), prim->Vertices.end());
			}
			// 頂点バッファ合成(スキニングデータ)
			if (prim->VerticesEx_Skin.size() >= 1) {
				destNode->Mesh.VerticesEx_Skin.insert(destNode->Mesh.VerticesEx_Skin.end(), prim->VerticesEx_Skin.begin(), prim->VerticesEx_Skin.end());
			}
			*/
			...
			// 
			currentVertexIdx += prim->VertexData.PosList.size();
	...
	for (UINT ani = 0; ani < model.animations.size(); ani++)
	{
		...
		for (const auto& channel : srcAni.channels)
		{
			...
			// ルートボーン？
			if (destModel->Nodes[channel.target_node].BoneNodeIndex == 0)
			{
				destAnimNode->IsRootBone = true;
			}
			...
			if (channel.target_path == "translation")
			{
				...
				for (UINT ki = 0; ki < timeGetter.GetAccessor()->count; ki++)
				{
					...
					if (v.Time > animation->GetMaxLength())
					{
						animation->SetMaxLength(v.Time);
					}
...
//						v.NextTangent.z = valueGetter.GetValue_Float(ki * 9 + 8);
			...
			else if (channel.target_path == "rotation")
			{
				...
				for (UINT ki = 0; ki < timeGetter.GetAccessor()->count; ki++)
				{
					...
					if (q.Time > animation->GetMaxLength())
					{
						animation->SetMaxLength(q.Time);
					}
...
//						q.NextTangent.w = valueGetter.GetValue_Float(ki * 12 + 11);
	...
	// アニメーションスクリプト
	//----------------------------------
	std::filesystem::path animScriptPath = path;
	animScriptPath = animScriptPath.parent_path() / animScriptPath.stem();
	animScriptPath += ".script";

	json11::Json jsn = KdLoadJSONFile(animScriptPath.string());
	if (jsn.is_null() == false)
	{
		auto& animations = jsn["Animations"].array_items();
		for (auto&& animJson : animations)
		{
			// 対象のアニメ名
			std::string animName = animJson["Name"].string_value();

			// データ
			std::vector<KdKeyScript> scripts;
			auto& scriptsJson = animJson["Scripts"].array_items();
			for (auto&& scrJson : scriptsJson)
			{
				KdKeyScript dest;
				dest.Time = (float)scrJson["Time"].number_value();
				dest.JsonData = scrJson;
				scripts.push_back(dest);
			}

			// セット
			for (auto&& n : destModel->Animations)
			{
				if (n->GetName() == animName)
				{
					n->SetScripts(scripts);
					break;
				}
			}
		}
