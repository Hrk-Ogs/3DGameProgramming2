...
public:
	...
	// 画像リソースを取得
	ID3D11Texture2D*					GetResource() const { return m_resource; }
	...
	// (テクスチャ配列時のみ)idx番目のシェーダーリソースビュー
	ID3D11ShaderResourceView*			GetSRView(int idx) const { return m_srvArray[idx]; }
	ID3D11ShaderResourceView* const*	GetSRViewAddress(int idx) const { return &m_srvArray[idx]; }

	// (テクスチャ配列時のみ)idx番目のレンダーターゲットリソースビュー
	ID3D11RenderTargetView*				GetRTView(int idx) const { return m_rtvArray[idx]; }
	ID3D11RenderTargetView* const*		GetRTViewAddress(int idx) const { return &m_rtvArray[idx]; }

	// (テクスチャ配列時のみ)idx番目の深度リソースビュー
	ID3D11DepthStencilView*				GetDSView(int idx) const { return m_dsvArray[idx]; }
	...
	// ・fillData	… バッファに書き込むデータ　nullptrだと書き込みなし
	// 戻り値：true … 成功
	bool CreateDepthStencil(int w, int h, DXGI_FORMAT format = DXGI_FORMAT_R24G8_TYPELESS, UINT arrayCnt = 1, const D3D11_SUBRESOURCE_DATA* fillData = nullptr);
...
private:
	...
	// 「テクスチャ配列」機能時の、各画像への個別アクセス用ビュー
	std::vector<ID3D11ShaderResourceView*>	m_srvArray;
	std::vector<ID3D11RenderTargetView*>	m_rtvArray;
	std::vector<ID3D11DepthStencilView*>	m_dsvArray;
